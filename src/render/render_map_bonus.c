...
/*   Updated: 2025/01/28 06:07:16 by jonathro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_pixel(t_vars *vars, int x, int y, unsigned long color)
{
	...
	if ((center_x + x) >= 0 && (center_x + x) < WIN_WIDTH
		&& (center_y + y) >= 0 && (center_y + y) < WIN_HEIGHT)
	{
		i = ((center_x + x) * (vars->mlx->bits_per_pixel / 8))
			+ ((center_y + y) * vars->mlx->size_line);
		if (i >= 0 && i + 2 < (WIN_WIDTH * WIN_HEIGHT * (vars->mlx->bits_per_pixel / 8)))
		{
