...
{
    int i;
    int adjusted_x = (WIN_WIDTH / 2) + x;
    int adjusted_y = (WIN_HEIGHT / 2) + y;

    if (adjusted_x >= 0 && adjusted_x < WIN_WIDTH && adjusted_y >= 0 && adjusted_y < WIN_HEIGHT)
    {
        i = (adjusted_x * vars->mlx->bits_per_pixel / 8) + (adjusted_y * vars->mlx->size_line);
...
static void draw_line(t_vars *vars, t_point p1, t_point p2, unsigned long color)
{
    ...
    double x_inc = (p2.x - p1.x) / (double)steps;
    double y_inc = (p2.y - p1.y) / (double)steps;
...
static void set_lines(t_vars *vars, int x, int y, unsigned long point)
{
    ...
    if (x > 0 && vars->map.data[y][x - 1] != ULONG_MAX)
    {
        ...
        draw_line(vars, curr_point, left_point, get_color(vars->map.data[y][x - 1]));
    }
    ...
    if (y > 0 && vars->map.data[y - 1][x] != ULONG_MAX)
    {
        ...
        draw_line(vars, curr_point, top_point, get_color(vars->map.data[y - 1][x]));
    }
}
